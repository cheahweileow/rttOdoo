...
static const st_ov_reg_t cam_config_table_normal_mode[] =
{
 ...
 { 0x300e, 0x00 }, // MIPI control, 1 lane, MIPI disabled before VIN starts
 ...
 { 0x3019, 0x70 }, // MIPI lanes and clock lane in LP11 while sleeping
...
static volatile uint32_t g_vin_frame_count = 0;
static volatile uint32_t g_mipi_callback_count = 0;
static volatile uint32_t g_vin_last_buffer = 0;
static volatile uint32_t g_vin_last_event = 0;
static volatile uint32_t g_vin_last_status = 0;
...
fsp_err_t camera_init (bool use_test_mode)
{
    ...
    if (!rdSensorReg16_8(REG_PRODUCT_ID_H, &reg_val1)) // PIDH  PID MSB
    {
        rt_kprintf("camera read product ID high failed\n");
        return FSP_ERR_HW_LOCKED;
    }

    if (!rdSensorReg16_8(REG_PRODUCT_ID_L, &reg_val2)) // PIDH  PID LSB REV2c - 0x4C, REV2a = 0x41, REV1a=0x40 otherwise error
    {
        rt_kprintf("camera read product ID low failed\n");
        return FSP_ERR_HW_LOCKED;
    }
...
static fsp_err_t ov5640_configure_clocks()
{
    ...
    uint8_t pll_multiplier = 84; // Bit[7:0]: PLL multiplier (4~252, even integer from 128+)
    uint8_t reg_3036_value = (uint8_t)(pll_multiplier);

    wrSensorReg16_8(0x3036, reg_3036_value);  // PLL Multiplier
...
fsp_err_t camera_capture_start(void)
{
    ...
    rt_kprintf("R_VIN_CaptureStart enter, buffer=%p\n", &camera_capture_image_rgb565[0]);
    fsp_err_t err = R_VIN_CaptureStart(&g_cam_vin_ctrl, &camera_capture_image_rgb565[0]);
    rt_kprintf("R_VIN_CaptureStart exit, err=%d\n", err);

    if (FSP_SUCCESS == err)
    {
        rt_thread_mdelay(5);
        ov5640_stream_on();
        rt_kprintf("OV5640 stream on\n");
    }
...
uint32_t camera_vin_frame_count_get(void)
{
    return g_vin_frame_count;
}

uint32_t camera_mipi_callback_count_get(void)
{
    return g_mipi_callback_count;
}

uint32_t camera_vin_last_buffer_get(void)
{
    return g_vin_last_buffer;
}

uint32_t camera_vin_last_event_get(void)
{
    return g_vin_last_event;
}

uint32_t camera_vin_last_status_get(void)
{
    return g_vin_last_status;
}
...
void cam_vin_callback(capture_callback_args_t *p_args)
{
    ...
    g_vin_last_event = (uint32_t) event;
    g_vin_last_status = p_args->interrupt_status;
    g_vin_last_buffer = (uint32_t) p_args->p_buffer;
